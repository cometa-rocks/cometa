...
export class DownloadService {
  ...
  downloadFile(response, file: UploadedFile) {
    const downloading = this._snack.open('Generating file to download, please be patient.', 'OK', { duration: 1000 })
    const blob = new Blob([this.base64ToArrayBuffer(response.body)], { type: file.mime });
