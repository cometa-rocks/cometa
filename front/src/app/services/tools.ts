...
export function getBrowserComboText(
  ...
  if (browser.mobile_emulation) {
    return `${formatVersion(titleCase(browser.device))} - ${formatVersion(
      titleCase(browser.os)
    )} ${formatVersion(browser.os_version)}`;
  }
  if (browser.device) {
    // Use device instead of browser if available
    return `${formatVersion(titleCase(browser.device))} ${titleCase(
      formatVersion(browser.browser_version)
    )} - ${formatVersion(titleCase(browser.os))} ${formatVersion(
      browser.os_version
    )}`;
  }
  if (so) {
    return `${formatVersion(titleCase(browser.browser))} ${titleCase(
      formatVersion(browser.browser_version)
    )} - ${formatVersion(titleCase(browser.os))} ${formatVersion(
      browser.os_version
    )}`;
