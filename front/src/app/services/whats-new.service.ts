...
export class WhatsNewService {
  ...
  collectChanges(): LogChange[] {
    ...
    const exists =
      !!previousVersion &&
      changelog.some(item => item.version === previousVersion);
    if (exists) {
      // Get indexes of current version and previous
      const currentVersionIndex = changelog.findIndex(
        item => item.version === currentVersion
      );
      const previousVersionIndex = changelog.findIndex(
        item => item.version === previousVersion
      );
      // Collect changes between versions
      const changelogChanges = changelog.slice(
        currentVersionIndex,
 ...
 collectAllChanges(): LogChange[] {
   ...
   */
  showAllWhatsNewDialog() {
    // Collect all changes
    const changes = this.collectAllChanges(); 
    // Open dialog with all changes
    this.openChangelog(changes); 
  }
}
