...
export class JoyrideStepsContainerService {
  ...
  addStep(stepToAdd: JoyrideStep) {
    let stepExist =
      this.tempSteps.filter(step => step.name === stepToAdd.name).length > 0;
    if (!stepExist) {
  ...
  get(action: StepActionType): JoyrideStep {
    ...
    if (this.currentStepIndex < 0 || this.currentStepIndex >= this.steps.length)
      throw new JoyrideStepOutOfRange(
    ...
    if (stepFound == null) {
      this.logger.warn(
        ...
        } not found in the DOM. Check if it's hidden by *ngIf directive.`
      );
    }
  ...
  getStepRoute(action: StepActionType) {
    ...
    if (action === StepActionType.NEXT) {
      stepID = this.steps[this.currentStepIndex + 1]
        ? this.steps[this.currentStepIndex + 1].id
        : null;
    } else {
      stepID = this.steps[this.currentStepIndex - 1]
        ? this.steps[this.currentStepIndex - 1].id
    ...
    let stepRoute =
      stepID && stepID.includes(ROUTE_SEPARATOR)
        ? stepID.split(ROUTE_SEPARATOR)[1]
        : '';
  ...
  private getStepIndex(stepName: string): number {
    const index = this.steps
      .map(step =>
        step.id.includes(ROUTE_SEPARATOR)
          ? step.id.split(ROUTE_SEPARATOR)[0]
          : step.id
      )
      .findIndex(name => stepName === name);
    if (index === -1)
      throw new JoyrideError(
        `The step with name: ${stepName} does not exist in the step list.`
      );
    return index;
  }
  ...
  private getStepName(stepID: string): string {
    let stepName =
      stepID && stepID.includes(ROUTE_SEPARATOR)
        ? stepID.split(ROUTE_SEPARATOR)[0]
        : stepID;
    return stepName;
  }
}
