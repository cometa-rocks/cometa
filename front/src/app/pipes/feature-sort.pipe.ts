...
export class FeatureSortPipe implements PipeTransform {
  ...
  transform(ids: number[], sort: string, reverse: boolean): Observable<number[]> {
  ...
  sortFn(sort, features: Feature[]) {
    ...
    switch (sort) {
      ...
      case 'feature_name':
        ...
        break;
      case 'execution':
        features.sort((a, b) => _amParse.transform(a?.info?.result_date).valueOf() - _amParse.transform(b?.info?.result_date).valueOf());
        features.reverse();
        break;
      case 'duration':
        features.sort((a, b) => parseInt(`${a?.info?.execution_time}`, 10) - parseInt(`${b?.info?.execution_time}`, 10));
        features.reverse();
        break;
      case 'status':
        features.sort((a, b) => {
      ...
      case 'last_edited':
        ...
        features.sort((a, b) => _amParse.transform(a?.last_edited_date).valueOf() - _amParse.transform(b?.last_edited_date).valueOf());
        features.reverse();
        break;
    }
  }
