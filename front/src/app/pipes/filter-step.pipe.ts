...
export class FilterStepPipe implements PipeTransform {
  ...
  transform(steps: Action[], search: string, templates: boolean): Action[] {
    ...
    const newSteps = templates ? steps.filter(step => step.action_name.includes('given')) : steps;
    return newSteps.filter(step => step.action_name.toLowerCase().includes(search));
  }
