...
export class IntegrationsState {
  ...
  constructor( private _api: ApiService ) { }
    ...
    getIntegrations({ setState }: StateContext<Integration[]>) {
        return this._api.getIntegrations().pipe(
            tap(ints => setState(ints))
        );
    }

    @Action(Integrations.Set)
    setIntegrations({ setState }: StateContext<Integration[]>, { integrations }: Integrations.Set) {
        setState(integrations);
    }

    @Action(Integrations.AddOne)
    addIntegration({ setState, getState }: StateContext<Integration[]>, { integration }: Integrations.AddOne) {
        setState(
    ...
    patchIntegration({ setState, getState }: StateContext<Integration[]>, { integration }: Integrations.PatchOne) {
        setState(
            produce(getState(), (ctx: Integration[]) => {
    ...
    removeIntegration({ setState, getState }: StateContext<Integration[]>, { integrationId }: Integrations.RemoveOne) {
        setState(
            produce(getState(), (ctx: Integration[]) => ctx.filter(int => int.id !== integrationId))
        )
    }
