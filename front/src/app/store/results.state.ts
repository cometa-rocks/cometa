...
@State<IResults>({
    name: 'results',
    defaults: {
        comment: 'This state manages the Live Steps Dialog, status of feature running and the notifications when a feature completes',
        notification_ids: []
    }
})
@Injectable()
export class ResultsState {
     ...
     * @returns {string}
     */
    getTimeoutKey(feature_id: number, run_id: number, browser_info: BrowserstackBrowser) {
     ...
     */
    clearTimeout(feature_id: number, run_id: number, browser_info: BrowserstackBrowser) {
     ...
     * @param {number} browser_info Browser information object
     */
    resetTimeout(feature_id: number, run_id: number, browser_info: BrowserstackBrowser) {
        this.clearTimeout(feature_id, run_id, browser_info);
        const timeoutKey = this.getTimeoutKey(feature_id, run_id, browser_info);
        // Get WebSockets Timeout from Config
        const { websocketsTimeout } = this._store.selectSnapshot<Config>(ConfigState);
        // Set timeout in subscription variable
        this.timeoutSubcriptions[timeoutKey] = timer(websocketsTimeout).subscribe(_ => {
            this._store.dispatch( new WebSockets.RunTimeout(feature_id, run_id, browser_info) );
     ...
     * @author Alex Barba
     */
    getSteps(state: IResults, feature_id: number, run_id: number, browser_info: BrowserstackBrowser) {
    ...
    modifySteps(
        ...
        state[feature_id].results[run_id][getBrowserKey(browser_info)].steps = newSteps;
        state[feature_id].results[run_id][getBrowserKey(browser_info)].status = status;
    }
    /**
     ...
     * @author Alex Barba
     */
    verifyAndFixMainKeys(state: IResults, feature_id: number, run_id: number, browser_info: BrowserstackBrowser, status?: string, feature_result_id?: number) {
        const browserKey = getBrowserKey(browser_info);
        // Check feature id
        ...
        if (!state[feature_id].results[run_id].hasOwnProperty(browserKey)) {
            ...
            // @ts-ignore
            state[feature_id].results[run_id][browserKey] = state[feature_id].results[run_id][browserKey] || {};
    ...
    setFeatureTimeout(
        ...
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Timeout');
            })
        );
    }
    ...
    setFeatureInitializing(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, feature_result_id}: WebSockets.FeatureInitializing
    ) {
        this.resetTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Initializing', feature_result_id);
    ...
    setFeatureFinished(
        ...
        { feature_id, run_id, browser_info, feature_result_info, feature_result_id }: WebSockets.FeatureFinished
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        // Notify to the subscribed clients
        if (getState().notification_ids.includes(feature_id)) {
            this._snack.open(`Browser ${getBrowserComboText(feature_result_info.browser)} for feature ${feature_result_info.feature_name} completed!`, 'VIEW', {
                duration: 5000,
        ...
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Completed', feature_result_id);
    ...
    setStepStarted(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, step_name, step_index, datetime, feature_result_id }: WebSockets.StepStarted
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, step_name, feature_result_id);
                const currentSteps = this.getSteps(ctx, feature_id, run_id, browser_info);
                // Check step exists
                if (typeof currentSteps[step_index] === 'undefined') {
                    // Mock data
                    currentSteps[step_index] = this.mockStep(step_index, step_name, datetime);
                }
    ...
    setStepDetail(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, step_index, datetime, info }: WebSockets.StepDetailedInfo
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
    ...
    setFeatureStarted(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, start_at, feature_result_id }: WebSockets.FeatureStarted
    ) {
        const browserKey = getBrowserKey(browser_info);
        this.resetTimeout(feature_id, run_id, browser_info);
        setState(
            produce((ctx: IResults) => {
                this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, 'Starting', feature_result_id);
    ...
    load({ patchState }: StateContext<IResults>) {
        try {
            const notifications = JSON.parse(localStorage.getItem('notifications') || '[]') as number[];
            patchState({
    ...
    addNotification(
        ...
        setState(
            produce((ctx: IResults) => {
                ...
                localStorage.setItem('notifications', JSON.stringify(ctx.notification_ids));
            })
        );
    }
    ...
    removeNotification(
        ...
        setState(
            produce((ctx: IResults) => {
                ctx.notification_ids = ctx.notification_ids.filter(id => id !== feature_id);
                localStorage.setItem('notifications', JSON.stringify(ctx.notification_ids));
            })
        );
    }
    ...
    setFeatureError(
        { setState }: StateContext<IResults>,
        { feature_id, run_id, browser_info, error, feature_result_id }: WebSockets.FeatureError
    ) {
        this.clearTimeout(feature_id, run_id, browser_info);
        const browserKey = getBrowserKey(browser_info);
        setState(
            produce((ctx: IResults) => {
                try {
                    this.verifyAndFixMainKeys(ctx, feature_id, run_id, browser_info, error, feature_result_id);
                    ctx[feature_id].running = false;
    ...
    static GetFeatureBrowserStatus(state: IResults) {
        return (feature_id: number, feature_run_id: number, browser: string) => {
            return feature_id in state && state[feature_id].results[feature_run_id][browser].status;
        };
    }
    ...
    static GetFeatureRunningStatus(state: IResults) {
        return (feature_id: number) => {
            return feature_id in state && (state[feature_id].running || state[feature_id].status === 'Queued');
        };
    }
    ...
    static GetLastFeatureRunDetails(state: IResults) {
        return (feature_id: number, run_id: number, browser: string, index: number) => {
