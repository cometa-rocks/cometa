...
export class BrowsersState {
  ...
  getBrowsers({ setState }: StateContext<BrowserResultObject[]>) {
    return this._api.getBrowsers().pipe(tap(browsers => setState(browsers)));
  }

  @Action(Browsers.SetBrowsers)
  setBrowsers(
    { setState }: StateContext<BrowserResultObject[]>,
    { browsers }: Browsers.SetBrowsers
  ) {
    setState(browsers);
  }
  ...
  updateBrowser(
    { setState, getState }: StateContext<BrowserResultObject[]>,
    { browser }: Browsers.UpdateBrowser
  ) {
    setState(
  ...
  removeBrowser(
    { setState, getState }: StateContext<BrowserResultObject[]>,
    { browser_id }: Browsers.RemoveBrowser
  ) {
