...
@State<IStepDefinitionsState>({
  ...
  defaults: {
      comment: 'This state saves the steps defined for each feature, including the new and cloned features.'
  }
})
@Injectable()
export class StepDefinitionsState {
    ...
    setAll({ setState }: StateContext<IStepDefinitionsState>, { featureId, steps }: StepDefinitions.SetStepsForFeature) {
        setState(
    ...
    getAll({ setState }: StateContext<IStepDefinitionsState>, { featureId }: StepDefinitions.GetStepsForFeature) {
