...
@State<IFeaturesState>({
  ...
  defaults: {
    ...
    departments: [],
    comment:
      'This state saves the information of each each feature the user has access to and the filters of the search view.',
  },
})
@Injectable()
export class FeaturesState {
  ...
  getSearch({ patchState }: StateContext<IFeaturesState>) {
    return this._api
      .getFolders()
      .pipe(tap(folders => patchState({ folders: folders })));
  }
  ...
  updateFeature(
    ...
    { feature_id }: Features.UpdateFeature
  ) {
  ...
  pushNewFeature(
    { setState }: StateContext<IFeaturesState>,
    { feature_id }: Features.PushNewFeatureId
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
  ...
  updateFeatureOffline(
    { setState }: StateContext<IFeaturesState>,
    { feature }: Features.UpdateFeatureOffline
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
  ...
  patchFeature(
    { setState }: StateContext<IFeaturesState>,
    { feature_id, info }: Features.PatchFeature
  ) {
  ...
  removeFeature(
    { setState }: StateContext<IFeaturesState>,
    { feature_id }: Features.RemoveFeature
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
  ...
  setMoreOrLessSteps(
    { patchState }: StateContext<IFeaturesState>,
    { moreOrLess }: Features.SetMoreOrLessSteps
  ) {
  ...
  addFilter(
    { setState, getState }: StateContext<IFeaturesState>,
    { filter }: Features.AddFilter
  ) {
  ...
  removeFilter(
    ...
    setState(
      produce((ctx: IFeaturesState) => {
        ...
        switch (filter.id) {
          case 'app':
            ctx.applications = ctx.applications.filter(
              app => app !== filter.value
            );
            break;
          case 'env':
            ctx.environments = ctx.environments.filter(
              env => env !== filter.value
            );
            break;
          case 'dept':
            ctx.departments = ctx.departments.filter(
              dept => dept !== filter.value
            );
            break;
        }
      })
  ...
  setFeatureInfo(
    { patchState, getState }: StateContext<IFeaturesState>,
    { features }: Features.SetFeatureInfo
  ) {
  ...
  modifyFeatureInfo(
    { setState }: StateContext<IFeaturesState>,
    { feature_id, property, value }: Features.ModifyFeatureInfo
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
  ...
  addFolderRoute(
    ...
    { folder }: Features.AddFolderRoute
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
    ...
    dispatch(
      new Paginations.ResetPagination([
        'search_with_depends',
        'search_without_depends',
      ])
    );
  }
  ...
  newAddFolderRoute(
    { setState, dispatch }: StateContext<IFeaturesState>,
    { folder }: Features.NewAddFolderRoute
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
        let departmentsList = this._store.select<Folder[]>(
          CustomSelectors.GetDepartmentFolders()
        ); // Get the list of existing departments
        let department;
        // Adds the department variable
        departmentsList.subscribe(
          val =>
            (department = val.filter(
              department => department.department == folder.department
            ))
        );
        ...
        if (folder) {
          ...
          if (!folder[0]?.type) {
            ...
            departmentsList.subscribe(
              val =>
                (department = val.filter(
                  department => department.department == folder[0]?.department
                ))
            );
  ...
  returnToFolderRoute(
    { patchState, getState, dispatch }: StateContext<IFeaturesState>,
    { folderId }: Features.ReturnToFolderRoute
  ) {
    ...
    // Check folder id
    if (folderId) {
      const indexOfName = currentRoute.findIndex(
        route => route.folder_id === folderId
      );
      // Remove the subsequent folders
      currentRoute.splice(
        indexOfName + 1,
        currentRoute.length - indexOfName + 1
      );
    } else {
  ...
  newReturnToFolderRoute(
    ...
    // Check folder id
    if (folderId) {
      const indexOfName = currentRouteNew.findIndex(
        route => route.folder_id === folderId
      );
      // Remove the subsequent folders
      currentRouteNew.splice(
        indexOfName + 1,
        currentRouteNew.length - indexOfName + 1
      );
    } else {
      // If folderId is 0 or null, it means is root folder
      currentRouteNew = [];
  ...
  setApplication(
    { patchState }: StateContext<IFeaturesState>,
    { app_id }: Features.SetApplicationFilter
  ) {
  ...
  setEnvironment(
    { patchState }: StateContext<IFeaturesState>,
    { environment_id }: Features.SetEnvironmentFilter
  ) {
  ...
  folderGotRemoved(
    { patchState, getState }: StateContext<IFeaturesState>,
    { folder_id }: Features.FolderGotRemoved
  ) {
  ...
  folderGotRenamed(
    { setState }: StateContext<IFeaturesState>,
    { folder }: Features.FolderGotRenamed
  ) {
    setState(
      produce((ctx: IFeaturesState) => {
        const index = ctx.currentRoute.findIndex(
          folder => folder.folder_id === folder.folder_id
        );
        if (index >= 0) {
          // Replace folder name
  ...
  static IsEasterEgg(state: IFeaturesState): boolean {
    ...
    return state.filters.some(
      filter =>
        filter.id === 'test' &&
        filter.value.toString().toLowerCase() === 'code is poetry'
    );
  }
  ...
  static IsFolderInRoute(state: IFeaturesState) {
    return (folder: Folder) => {
      return state.currentRoute.some(
        route => route.folder_id === folder.folder_id
      );
    };
  }
  ...
  static GetSelectionFolders(state: IFeaturesState) {
    ...
    return !location.hash.includes('#/new')
      ? state.currentRoute
      : state.currentRouteNew;
  }
      ...
      selectedFolders.forEach(value => {
        if (value.type === 'department') {
          ...
          folders.features = folders.features.filter(
            f => state.details[f].department_id === value.folder_id
          );
        } else {
  ...
  static processFilters(
    ...
    if (departments.length > 0)
      features = features.filter(feature =>
        departments.includes(details[feature].department_id)
      );
    if (applications.length > 0)
      features = features.filter(feature =>
        applications.includes(details[feature].app_id)
      );
    if (environments.length > 0)
      features = features.filter(feature =>
        environments.includes(details[feature].environment_id)
      );
    ...
    state.filters.forEach(filter => {
      switch (filter.id) {
        ...
        case 'pixel_diff':
          ...
          features = this.computeEvaluation(
            filter.id,
            features,
            details,
            filter.value,
  ...
  static GetNewFeaturesWithinFolder(state: IFeaturesState, user): any {
    ...
    switch (
      activeList // Switch case to control which data to show
    ) {
      case 'list':
   ...
   * @author dph000
   */
  static getCurrentDirectoryData(
    state: IFeaturesState,
    ...
    if (currentDirectory.length > 0) {
      ...
      if (
        ...
        currentDirectory[0].type === 'department'
      ) {
   ...
   * @author dph000
   */
  static transformCurrentDirectoryData(
    result,
    state: IFeaturesState,
    folders: FoldersResponse
  ) {
  ...
  static getRecursiveData(
    ...
    folder.features.forEach(val => {
      if (
        state.details[val]?.feature_name.toLowerCase().includes(filter_name) ||
        state.details[val]?.feature_id.toString().includes(filter_name)
      ) {
    ...
    if (
      folder.name.toLowerCase().includes(filter_name) ||
      folder.folder_id.toString().includes(filter_name)
    ) {
      folder.route = route;
      folder.folders = [];
   ...
   * @lastModification 01-10-21
   */
  static newProcessFilters(
    state: IFeaturesState,
    search,
    folderList: FoldersResponse
  ): FoldersResponse {
  ...
  static GetFoldersWithinFolder(state: IFeaturesState): Folder[] {
    ...
    if (selectedFolders.length === 0) {
      ...
      selectedFolders.forEach(value => {
        if (value.type === 'department') {
          // Filter features by selected department folder
          features.folders = features.folders.filter(
            f => f.department === value.folder_id
          );
        } else {
          const index = features.folders.findIndex(
            folder => folder.folder_id === value.folder_id
          );
    ...
    if (departments.length > 0) {
      folders = folders.filter(folder =>
        departments.includes(folder.department)
      );
    }
    return folders;
  }
  ...
  static computeEvaluation(
    ...
    if (fields.includes(field)) field = 'info.' + field;
    switch (moreOrLessSteps) {
      case '>':
        return features.filter(
          feature =>
            getDescendantProp(details[feature], field) >
