...
export class RunsState {
    ...
    @Action(Runs.SetRuns)
    getAll({ setState }: StateContext<IRunsState>, { featureId, runs }: Runs.SetRuns) {
        setState(
            produce((ctx: IRunsState) => {
                ctx[featureId] = runs;
            })
    ...
    updateRunOffline({ setState }: StateContext<IRunsState>, { featureId, featureResult, runId }: Runs.UpdateRunOffline) {
        setState(
            produce((ctx: IRunsState) => {
                ...
                if (ref) {
                    ...
                    let resultIndex = ref[runIndex].feature_results.findIndex(res => res.feature_result_id === featureResult.feature_result_id);
                    if (resultIndex === -1) {
    ...
    updateRunArchived({ setState }: StateContext<IRunsState>, { featureId, runId, archived }: Runs.UpdateRunArchivedStatus) {
        setState(
            produce((ctx: IRunsState) => {
    ...
    updateFeatureResultArchived({ setState }: StateContext<IRunsState>, { featureId, runId, featureResultId, archived }: Runs.UpdateFeatureResultArchivedStatus) {
        setState(
            produce((ctx: IRunsState) => {
                // Get reference to current feature runs
                let runRef = ctx[featureId];
                ...
                const resultIndex = runRef[runIndex].feature_results.findIndex(res => res.feature_result_id === featureResultId);
                // Assign archived value
    ...
    overrideFeatureResultStatus({ setState }: StateContext<IRunsState>, { featureId, runId, resultId, status }: Runs.OverrideFeatureResultStatus) {
                ...
                if (res.success) {
                    ...
                    setState(
                        produce((ctx: IRunsState) => {
                            ...
                            const runIndex = ctx[featureId].findIndex(run => run.run_id === runId);
                            if (runIndex !== -1) {
                                // Find Result Index to modify
                                const resultIndex = ctx[featureId][runIndex].feature_results.findIndex(result => result.feature_result_id === resultId);
                                if (resultIndex !== -1) {
                                    // Update status
                                    ctx[featureId][runIndex].feature_results[resultIndex].status = status;
                                }
                            }
                        })
                    )
    ...
    overrideFeatureRunStatus({ setState }: StateContext<IRunsState>, { featureId, runId, status }: Runs.OverrideFeatureRunStatus) {
