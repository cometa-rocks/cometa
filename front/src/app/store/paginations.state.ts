...
export class PaginationsState {
    ...
    set({ setState }: StateContext<IPaginationsState>, { paginationId, pagination }: Paginations.SetPagination) {
        setState(
            produce((ctx: IPaginationsState) => {
                ...
                localStorage.setItem(`pagination.size.${paginationId}`, pagination.pageSize.toString());
            })
        )
    }

    @Action(Paginations.ResetPagination)
    reset({ setState }: StateContext<IPaginationsState>, { paginationId }: Paginations.ResetPagination) {
        setState(
            produce((ctx: IPaginationsState) => {
