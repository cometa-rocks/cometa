...
export class StepEditorComponent extends SubSinkAdapter implements OnInit {
  ...
  onTextareaArrowKey(event: Event, direction: string, step) {
    // Only prevent default and handle arrow keys if there are displayed variables AND no modifier keys
    const keyboardEvent = event as KeyboardEvent;
    const hasModifierKeys = keyboardEvent.ctrlKey || keyboardEvent.altKey || keyboardEvent.metaKey;
    
    if (this.displayedVariables.length > 0 && !hasModifierKeys) {
    ...
    // If no variables are displayed or modifier keys are pressed, let the event continue to other listeners
  }
  ...
  insertStep(event: KeyboardEvent, i: number){
    ...
    // Only apply blur/focus logic for Ctrl+Up combination
    if(event.key == 'ArrowUp'){
      // Remove focus from textarea to avoid event conflicts
      const textarea = event.target as HTMLTextAreaElement;
      textarea.blur();
      ...
      // Restore focus to the textarea after a short delay
      setTimeout(() => {
        textarea.focus();
      }, 10);
    } else if(event.key == 'ArrowDown'){
      // For Ctrl+Down, use normal behavior without blur/focus
      // Cerrar el autocompletado
      const autocompletePanel = document.querySelector('.mat-autocomplete-panel');
      if (autocompletePanel) {
          autocompletePanel.remove();
  ...
  copyStep(event: KeyboardEvent, i: number){
    ...
    // Only apply blur/focus logic for Ctrl+Alt+Up combination
    if(event.key == 'ArrowUp'){
      // Remove focus from textarea to avoid event conflicts
      const textarea = event.target as HTMLTextAreaElement;
      textarea.blur();
      ...
      // For Ctrl+Alt+Down, use normal behavior without blur/focus
      // Cerrar el autocompletado
      const autocompletePanel = document.querySelector('.mat-autocomplete-panel');
      if (autocompletePanel) {
          autocompletePanel.remove();
