...
export class EnvironmentComponent {
  ...
  canEditEnvironment$: Observable<boolean>;
  @Select(UserState.GetPermission('delete_environment'))
  canDeleteEnvironment$: Observable<boolean>;
    ...
    if (this.modify.getValue()) {
      this._api
        .renameEnvironment(
          ...
          this.environment.environment_name
        )
        .subscribe(
          res => {
            if (res.success)
              this._snack.open('Environment renamed successfully!', 'OK');
  ...
  removeIt() {
    this._dialog
      ...
      .subscribe(answer => {
        if (answer)
          this._api
            .deleteEnvironment(this.environment.environment_id)
            .subscribe(
              res => {
                if (res.success) {
                  this._store.dispatch(
                    new Environments.RemoveEnvironment(
                      this.environment.environment_id
                    )
                  );
