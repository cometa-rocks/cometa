...
export class ApplicationComponent {
  ...
  canEditApplication$: Observable<boolean>;
  @Select(UserState.GetPermission('delete_application'))
  canDeleteApplication$: Observable<boolean>;
  ...
  saveOrEdit() {
    if (this.modify.getValue()) {
      this._api.renameApplication(this.app.app_id, this.app.app_name).subscribe(
        res => {
          if (res.success)
            this._snack.open('Application renamed successfully!', 'OK');
  ...
  removeIt() {
    this._dialog
      ...
      .subscribe(answer => {
        if (answer)
          this._api.deleteApplication(this.app.app_id).subscribe(
            res => {
              if (res.success) {
                this._store.dispatch(
                  new Applications.RemoveApplication(this.app.app_id)
                );
