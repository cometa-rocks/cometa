...
export class DetailViewComponent implements OnInit {
  ...
  @ViewSelectSnapshot(UserState.GetPermission('remove_screenshot')) canDeleteScreenshot: boolean;
  ...
  previous() {
    ...
    if (previousStepId) {
      this._router.navigate(['../../detail', previousStepId], { relativeTo: this._acRouted }).then(() => window.scrollTo(0, 0));
    }
  }
  ...
  next() {
    ...
    if (nextStepId) {
      this._router.navigate(['../../detail', nextStepId], { relativeTo: this._acRouted }).then(() => window.scrollTo(0, 0));
    }
  }
  ...
  removeScreenshot(type: ScreenshotType) {
    ...
    .pipe(
      ...
      switchMap(step => this._api.removeScreenshot(step.step_result_id, type).pipe(
    ...
    .subscribe(step => {
      if (type === 'style') {
        ...
        const templateFile = step.screenshot_template || step.template_name || '';
        if (templateFile) {
          // Remove template style image if is of type template and exists
          this._api.removeTemplate(step.step_result_id, templateFile).subscribe(res => {
            if (!res.success) {
              console.log('An error ocurred while removing screenshot', res);
            }
          }, err => {
            console.log(err);
          });
  ...
  getIndexImage(type: ScreenshotType): SafeStyle {
    ...
    return this._sanitizer.bypassSecurityTrustStyle(`url(${this.api_base}screenshot/${screenshot}/)`);
  }
  ...
  ngOnInit() {
    ...
    this._acRouted.paramMap.pipe(
      map(params => +params.get('feature_result_id')),
      distinctUntilChanged(),
      tap(featureResultId => this.featureResult$ = this._store.select(CustomSelectors.GetFeatureResultById(featureResultId))),
      switchMap(featureResultId => this._store.dispatch( new FeatureResults.GetFeatureResult(featureResultId, true) ))
