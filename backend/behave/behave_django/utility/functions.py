...
def get_model(obj, model):
    ...
    raise Exception(
        "Unable to resolve %s with argument type %s" % (str(model), type(obj))
    )
...
def toWebP(image):
    ...
    if os.path.isfile(compressedImage):
        logger.debug(
            "Ckecked using os.path.isfile that File was save on disk ok - %s"
            % compressedImage
        )
    else:
    ...
    logger.debug(
        "Checking again using os.path.isfile that File was save on disk ok - %s"
        % compressedImage
    )
    if os.path.isfile(compressedImage):
...
def getStepResultScreenshotsPath(
    featureId, runId, featureResultId, runHash, stepResultId, **kwargs
):
    # Get prefix keyword
...
def set_test_schedule(feature_id, schedule, user_id):
    post_data = {"feature_id": feature_id, "schedule": schedule, "user_id": user_id}
...
def calculate_run_totals(run):
    ...
    for field in fields:
        setattr(
            run,
            field,
            run.feature_results.all().aggregate(total=Coalesce(Sum(field), Value(0)))[
                "total"
            ],
        )
    run.save()
...
def get_nested_dict_property(obj, properties):
    if not isinstance(obj, dict) and not isinstance(obj, list):
        raise Exception(
            "Invalid dict/list object passed. Must be a dict or list. Passed: %s"
            % type(obj)
        )
