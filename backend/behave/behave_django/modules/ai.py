...
class AI:
    ...
    def analyze_image(self,context, data):
        try:
            queue = Queue(self.REDIS_IMAGE_ANALYSYS_QUEUE_NAME, connection = self.__REDIS_CONNECTION)
            # Enqueue the task
            ...
            while worker_job.is_queued or worker_job.is_scheduled or worker_job.is_started:
                send_step_details(context, f"Analyzing image, status {worker_job.get_status()}")
