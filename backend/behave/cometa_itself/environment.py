...
# Importing should be done after the load_configurations so configuration values are available
...
def before_all(context):
    ...
    context.cloud = context.browser_info.get("cloud", "browserstack")
    ...
    # Do NOT start or connect to browser here. All browser startup logic is now in BrowserDriverManager and only called from the step.
    # The rest of the context setup (non-browser) remains as before.
...
def after_all(context):
    ...
    BrowserDriverManager.stop_browser(context)
    # (Keep all the rest of the after_all logic as before, but skip browser quit logic)
