...
class ResponseManager:
    ...
    def id_not_found_error_response(self, id, app_name=None):
        ...
        data["success"] = False
        data["errors"] = (
            f"{app_name if app_name is not None else self.__app_name} with id {id} dose not exist"
        )
        return JsonResponse(data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def user_not_found_error_response(self, id, app_name=None):
        ...
        data["success"] = False
        data["errors"] = (
            f"{app_name if app_name is not None else self.__app_name} with username {id} dose not exist"
        )
        return JsonResponse(data, status=status.HTTP_400_BAD_REQUEST)
    ...
    def created_response(self, data):
        return JsonResponse(
            {
                "success": True,
                "Result": f"{self.__app_name} Created",
                self.__app_name.lower(): data,
                "module": self.__app_name,
            },
    ...
    def updated_response(self, data):
        return JsonResponse(
            {
                "success": True,
                "Result": f"{self.__app_name} Updated",
                self.__app_name.lower(): data,
                "module": self.__app_name,
            },
            status=status.HTTP_201_CREATED,
    ...
    def deleted_response(self, id):
        return JsonResponse(
            {"success": True, "message": f"{self.__app_name} with id '{id}' deleted"},
    ...
    def get_response(self, dict_data: dict = None, list_data: list = None):
        if dict_data is not None:
            return JsonResponse(
                {"success": True, self.__app_name.lower(): dict_data},
                status=status.HTTP_200_OK,
            )
        else:
            return JsonResponse(
    ...
    def not_implemented_response(self):
        return JsonResponse(
            {"error": "Endpoint/Method not implemented"}, status=status.HTTP_200_OK
        )
