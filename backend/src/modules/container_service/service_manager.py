...
class DockerServiceManager:
    ...
    def wait_for_service_to_be_running(
        ...
        logger.info(f"Waiting for service to be running with max_wait_time_seconds : {max_wait_time_seconds} with container_name_or_id : {service_name_or_id}")
        ...
        if time.time() - start_time > max_wait_time_seconds:
            return False, f"Service is not running waited for {max_wait_time_seconds} seconds, container_name_or_id : {service_name_or_id}"
    ...
    def restart_service(self, service_name_or_id, *args, **kwargs):
        ...
        try:
            ...
            container = self.docker_client.containers.get(service_name_or_id)
            container.restart()
            self.wait_for_service_to_be_running(service_name_or_id, max_wait_time_seconds)
            return True, container.attrs
        except Exception as e:
            logger.info(f"Can not restart the service with container_name_or_id : {service_name_or_id}, error message {str(e)}")
            traceback.print_exc() 
            return False, str(e)
    ...
    def stop_service(self, service_name_or_id, *args, **kwargs):
        ...
        try:
            ...
            container = self.docker_client.containers.get(service_name_or_id)
            container.stop()
            logger.info(f"Container stopped with container_name_or_id : {service_name_or_id}")

            return True, container.attrs
        except (NullResource, NotFound) as null_resource:
            logger.info(f"Can not stop service with container_name_or_id : {service_name_or_id}, error message {str(null_resource)}")
    ...
    def delete_service(self, service_name_or_id):
        ...
        try:
            ...
            container.remove()
            logger.info(f"Container removed with container_name_or_id : {service_name_or_id}")
            return True, "Container removed"
        except (NullResource, NotFound) as null_resource:
            logger.info(f"Can not delete service with container_name_or_id : {service_name_or_id}, error message {str(null_resource)}")
            return True, str(null_resource)
        except Exception as e:
            traceback.print_exc() 
            return False, str(e)
...
if ConfigurationManager.get_configuration("COMETA_DEPLOYMENT_ENVIRONMENT","docker") == "kubernetes":
    service_manager = KubernetesServiceManager
    logger.debug(f'Deployment type is {ConfigurationManager.get_configuration("COMETA_DEPLOYMENT_ENVIRONMENT","docker")}')
...
class ServiceManager(service_manager):
    ...
    def create_service(self, *args, **kwargs):
        ...
        return super().create_service(configuration=self.__service_configuration, *args, **kwargs)

    # This method will create the container base on the environment
    def restart_service(self, service_name_or_id, *args, **kwargs):
        return super().restart_service(service_name_or_id)

    # This method will create the container base on the environment
    def stop_service(self, service_name_or_id, *args, **kwargs):
        return super().stop_service(service_name_or_id)

    def delete_service(self, service_name_or_id):
        return super().delete_service(service_name_or_id)
